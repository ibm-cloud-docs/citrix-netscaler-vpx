...
In most deployments, the {{site.data.keyword.vpx_full}} appliance is deployed in a reverse-proxy configuration. However, to configure the VPX with a forward-proxy configuration, Virtual IP addresses (VIPs) are needed because the configuration exists on a private network instead of a public one.
{: shortdesc}
