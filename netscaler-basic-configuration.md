...
3. The NetScaler receives the traffic on that VIP, and makes note of the traffic's protocol that is being used (HTTP port 80 traffic).
...
To accomplish this setup, you must configure the NetScaler to handle this traffic. Because the VIP, the DNS server's IP, and the SNIP are already configured, this preexisting configuration simplifies the process.
...
5. Make sure that the **Traffic Domain** field is blank because you are only concerned with using the default traffic domain in this scenario.
...
4. Select the protocol that you are balancing (HTTP).
5. Ensure that the IP address type is set to IP address. The IP address field is where you enter the VIP that you are using as the entry point for all of your users.
6. Specify the port number. The default value is 80.
7. Click **OK** to create the virtual server.
...
With this process, you create a load balancing pool and policy for your website.
