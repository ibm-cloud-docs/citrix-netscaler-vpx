...
2. Using the Certificate Management Utility (CMU) in VPX, create a key pair by using the command that is shown in the following example. Make sure to use the designated partition password.
...
4. With the keys created in the previous step, generate a CSR with the CMU utility.
   ...
   However, in the output that is listed, the file name can be anything with a .csr extension a meaningful description is recommended.
